...
set video_encoder=bin\x264.2579kMod.x86.exe 
...
echo Loadplugin^("C:\Program Files (x86)\AviSynth\plugins\yadifmod.dll"^) > "%%~nf.avs"
echo Loadplugin^("C:\Program Files (x86)\AviSynth\plugins\nnedi3.dll"^) >> "%%~nf.avs"
echo LoadPlugin^("C:\Program Files (x86)\AviSynth\plugins\ffms2.dll"^) >> "%%~nf.avs"
echo FFVideoSource^("%%~f"^) >> "%%~nf.avs"
echo nnedi3^(field=1^) >> "%%~nf.avs" 

rem echo Encoding %%~f pass 1
rem echo %video_encoder_keys%
rem %video_encoder% %%~nf.avs %video_encoder_keys% --pass 1 --qp 25 --ipratio 1.33 --pbratio 1.33 --output NUL --stats %out_folder%%%~nf.stats --index %%~nf.index 
echo Encoding %%~f pass last
%video_encoder% %%~nf.avs %video_encoder_keys% --qp 25 --ipratio 1.4 --pbratio 1.33 --output %out_folder%%%~nf.264 --index %%~nf.index 
